...
import OSCEMed2 from './hp/osce/OSCEMed2';
import OSCEMed4 from './hp/osce/OSCEMed4';
import OSCEMed8 from './hp/osce/OSCEMed8';
...
const HPView: React.FC<HPViewProps> = ({ isPremium, onNavigateToPremium, onAddActivity, onAwardPoints, onIncrementUsage, userStats }) => {
  ...
  const [viewMode, setViewMode] = useState<'modules' | 'osce'>('modules');
  ...
  if (selectedMed) {
    return (
      <div className="max-w-[1200px] mx-auto animate-in fade-in duration-500 px-4">
        ...
        {viewMode === 'osce' ? (
          <>
            {selectedMed === 2 && <OSCEMed2 />}
            {selectedMed === 4 && <OSCEMed4 />}
            {selectedMed === 8 && <OSCEMed8 />}
          </>
        ) : (
          <>
