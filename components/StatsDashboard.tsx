...
const StatsDashboard: React.FC<StatsDashboardProps> = ({ stats, allUsers, onNavigate }) => {
  ...
  const quickActions = [
    ...
    { id: 'biblioteca', label: 'Biblioteca', icon: '📖', bgClass: 'bg-emerald-400/10', textClass: 'text-emerald-400' },
    { id: 'anamnese', label: 'Anamnese', icon: '📋', bgClass: 'bg-sky-500/10', textClass: 'text-sky-500', isSpecial: true, subtitle: 'Modelos estruturados' },
  ];
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {quickActions.map(action => (
          <button 
            ...
            className={`flex items-center gap-4 p-4 bg-white dark:bg-nexus-card border rounded-xl hover:bg-neutral-50 dark:hover:bg-nexus-surface hover:shadow-md dark:hover:shadow-none transition-all duration-200 group ${
              (action as any).isSpecial 
                ? 'border-sky-500/30 dark:border-sky-500/20 bg-sky-50/30 dark:bg-sky-500/5' 
                : 'border-neutral-200 dark:border-nexus-border hover:border-sky-500/30 dark:hover:border-nexus-hover'
            }`}
          >
            ...
            <div className="text-left flex-grow">
              <span className="block text-[9px] font-semibold text-neutral-400 dark:text-nexus-text-label uppercase tracking-widest mb-0.5">
                {(action as any).subtitle || 'Ir para'}
              </span>
